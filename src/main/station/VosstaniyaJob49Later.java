...
public class VosstaniyaJob49Later {
    ...
    private static Pattern pattern49 = Pattern.compile("(?<=(49\\s.{16}\\s\\d{2}:\\d{2}))(.+)");
    ...
    private static Pattern pattern49Later = Pattern.compile("49\\s\\D+\\s\\d{2}:\\d{2}");

    private static String getDateFromString49Later(String stringDate) throws Exception {
        Matcher matcher = pattern49Later.matcher(stringDate);
        if (matcher.find()) {
            return matcher.group();
        }
        throw new Exception("Can't extract date from string!");
    ...
    public static String getVosstaniyaJob49Later() throws Exception {
        ...
        String bus49Later = getDateFromString49Later(bus49);
        String bus491 = getDateFromStringNumber(bus49Later);
        String bus492 = getDateFromStringTime(bus49Later);
